...
export async function addClickEvent(event: ClickEvent) {
  try {
    ...
    await sql`
      ...
      VALUES (${category}, ${label}, ${url}, ${timestamp})
    `;
...
export async function getClickEvents(): Promise<ClicksData> {
  try {
    // Get all events
    const eventsResult = await sql`SELECT * FROM clicks ORDER BY timestamp DESC`;
    ...
    const labelCountsResult = await sql`
    ...
    const urlCountsResult = await sql`
